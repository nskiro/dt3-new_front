...
import { Calendar, Row, Col, Card, Alert, Divider, Tag, Select, Spin, message, Checkbox } from 'antd'
...
class ExcelViewer extends Component {
    ...
    onSelectReport = value => {
        ...
        axios.get(`api/pdf/report/readexcel/${value}`)
            .then(res => {
                this.setState({
                    loadingReport: false,
                    rows: res.data.data,
    ...
    render() {
        ...
        return (
            <span>
                ...
                <Row>
                    {checkboxColumns.map((value) => { return <Col xs={6} sm={3} key={value.key}><Checkbox value={value.key} onChange={this.onCheckedChange}>{value.name}</Checkbox></Col> })}
                </Row>
